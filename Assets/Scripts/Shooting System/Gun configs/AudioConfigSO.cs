...
public class AudioConfigSO : ScriptableObject, ICloneable
{
    ...
    public object Clone()
    {
        var config = CreateInstance<AudioConfigSO>();
    ...
    {
        if (EmptyClip != null) AudioSource.PlayOneShot(EmptyClip, Volume);
    }

    public void PlayReloadClip(AudioSource AudioSource)
    {
        if (ReloadClip != null) AudioSource.PlayOneShot(ReloadClip, Volume);
    }
}