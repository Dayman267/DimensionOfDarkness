...
public class PlayerController : MonoBehaviour
{
    ...
    private static bool isRightClickDown;
    private static bool isLeftClickDown;
    private static bool isLShiftDown;
    private static bool isRKeyDown;
    private static bool isQKeyDown;

    private static bool isEKeyDown;
    ...
    private void Update()
    {
        ...
        var inMovement = Mathf.Abs(direction.x) > 0 || Mathf.Abs(direction.z) > 0;
        ...
        {
            ListenWASDKeyUp();
            if (isRotating) RotateTowardsTarget();
        }
    ...
    private Vector3 CalculateMovementVector()
    {
        ...
        var cameraR = _mainCamera.right;
        var cameraF = _mainCamera.forward;
        ...
        var movementVector = cameraF.normalized * v + cameraR.normalized * h;
        movementVector = Vector3.ClampMagnitude(movementVector, maxMovementMagnitude);

        var targetMagnitude = isLShiftDown ? 1.5f : 1f;
        var lerpedMagnitude = Mathf.MoveTowards(_movementVector.magnitude, targetMagnitude, 1.1f * Time.deltaTime);
        movementVector = movementVector.normalized * lerpedMagnitude;

        var relativeVector = transform.InverseTransformDirection(movementVector);
    ...
    public void Move(bool isMoving)
    {
        ...
        if (isMoving)
        {
            ...
            var currentSpeed = isLShiftDown ? speed * speedIncreaseFactor : speed;
